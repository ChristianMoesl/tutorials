const fs = require('fs');
function printUsage() {
  console.error(`usage: node ${process.argv[1]} arg0`);
  process.exit(1);
}
...
.then(results => {
  ...
  const arg0 = 61;
  const arg1 = insertAt(new Uint8Array(mem.buffer), arg0, process.argv[2]);
  const returned = results.instance.exports.Snippet(arg0);
