...
object query_optwasm {
  trait QueryCompiler extends Dsl with StagedQueryProcessor with WasmScannerBase {
    override def version = "query_optwasm"
...
--------------------
    ...
    class Scanner(name: Rep[String]) {
      ...
      def next(d: Rep[Char]) = {
        ...
        pos += 1
        RString(stringPtrFromCharArray(data, start), len)
      }
...
----------------------------------
    ...
    def evalRef(r: Ref)(rec: Record): RField = r match {
      ...
      case Value(x) => RString(stringPtrFromString(x.toString), x.toString.length)
    }
...
------------------------------
    ...
    case class StringColBuffer(data: Rep[Array[StringPointer]], len: Rep[Array[Int]]) extends ColBuffer

    class ArrayBuffer(dataSize: Int, schema: Schema) {
      val buf = schema.map {
        case hd if isNumericCol(hd) => IntColBuffer(NewArray[Int](dataSize))
        case _ => StringColBuffer(NewArray[StringPointer](dataSize), NewArray[Int](dataSize))
      }
      var len = 0
      def +=(x: Fields) = {
        this(len) = x
